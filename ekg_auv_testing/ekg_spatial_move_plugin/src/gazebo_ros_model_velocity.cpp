...
namespace gazebo 
{
      ...
      sdf::ElementPtr sdf) 
  {
    ...
    limiter_lin_ = gazebo_beacon_plugin::SpeedLimiter();
    limiter_ang_ = gazebo_beacon_plugin::SpeedLimiter();
  ...
  void GazeboRosModelVelocity::UpdateChild()
  {
    ...
    if (dt > ros::Duration(1.0 / update_rate_)){
      ...
      parent_->SetLinearVel(ignition::math::Vector3(x_vel_cmd, y_vel_cmd, 0.0));
      parent_->SetAngularVel(ignition::math::Vector3(0.0, 0.0, cmd.angular.z));
