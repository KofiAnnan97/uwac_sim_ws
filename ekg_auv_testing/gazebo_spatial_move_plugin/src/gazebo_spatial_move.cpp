...
namespace gazebo 
{
  ...
  GazeboROSSpatialMove::GazeboROSSpatialMove() {}

  GazeboROSSpatialMove::~GazeboROSSpatialMove() {}

  // Load the controller
  void GazeboROSSpatialMove::Load(physics::ModelPtr parent, 
      sdf::ElementPtr sdf) 
  {
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&GazeboROSSpatialMove::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
    ...
    callback_queue_thread_ = 
      boost::thread(boost::bind(&GazeboROSSpatialMove::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ = 
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboROSSpatialMove::UpdateChild, this));

  }

  // Update the controller
  void GazeboROSSpatialMove::UpdateChild() 
  {
    boost::mutex::scoped_lock scoped_lock(lock);
    ignition::math::Pose3d pose = parent_->WorldPose();
    float yaw = pose.Rot().Yaw();
    float pitch = pose.Rot().Pitch();
    parent_->SetLinearVel(ignition::math::Vector3(
          x_ * cosf(yaw) - y_ * sinf(yaw), 
          y_ * cosf(yaw) + x_ * sinf(yaw), 
          0.0));                                                       //Change this to update the linear z velocity ( x_ * cosf(pitch) - z_ * sinf(pitch))
    parent_->SetAngularVel(ignition::math::Vector3(0.0, 0.0, rot_));
  ...
  void GazeboROSSpatialMove::FiniChild() {
    alive_ = false;
    queue_.clear();
    queue_.disable();
    rosnode_->shutdown();
  ...
  void GazeboROSSpatialMove::cmdVelCallback(
  ...
  void GazeboROSSpatialMove::QueueThread() 
  {
  ...
  void GazeboROSSpatialMove::publishOdometry(double step_time) 
  {
  ...
  // GZ_REGISTER_MODEL_PLUGIN(GazeboROSSpatialMove)
}