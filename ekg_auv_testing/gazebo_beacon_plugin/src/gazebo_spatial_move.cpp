...
namespace gazebo 
{
  ...
  GazeboSpatialMove::GazeboSpatialMove() {}

  GazeboSpatialMove::~GazeboSpatialMove() {}

  // Load the controller
  void GazeboSpatialMove::Load(physics::ModelPtr parent, 
      sdf::ElementPtr sdf) 
  {
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&GazeboSpatialMove::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
    ...
    callback_queue_thread_ = 
      boost::thread(boost::bind(&GazeboSpatialMove::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ = 
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboSpatialMove::UpdateChild, this));

  }

  // Update the controller
  void GazeboSpatialMove::UpdateChild() 
  {
    ...
    parent_->SetLinearVel(ignition::math::Vector3(
          ...
          0));
    parent_->SetAngularVel(ignition::math::Vector3(0, 0, rot_));
  ...
  void GazeboSpatialMove::FiniChild() {
    alive_ = false;
    queue_.clear();
    queue_.disable();
    rosnode_->shutdown();
  ...
  void GazeboSpatialMove::cmdVelCallback(
  ...
  void GazeboSpatialMove::QueueThread() 
  {
  ...
  void GazeboSpatialMove::publishOdometry(double step_time) 
  {
  ...
  GZ_REGISTER_MODEL_PLUGIN(GazeboSpatialMove)
}