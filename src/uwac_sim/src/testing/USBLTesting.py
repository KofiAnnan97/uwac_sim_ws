...
from uwac_sim.msg import USBLRequestSim, USBLResponseSim, VehiclePose, Packet, Payload, bcn_pose
...
class Transceiver():
    ...
    def send_command_response(self, msg):
        ...
        loaded = self.load_data('uwac_sim/USBLResponseSim', resp_msg)
        ...
        if msg is not None:
            ...
            if type == 'std_msgs/String':
                if processed.data == 'ping':
                    # rospy.loginfo(f'Pinging {self.transponder_id}')
                    self.query_location()
            elif type == 'uwac_sim/bcn_frame_array':
