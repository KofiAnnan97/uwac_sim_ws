...
from uwac_sim.msg import Payload, Packet, VehiclePose, USBLRequestSim, USBLResponseSim, bcn_frame_array, bcn_frame, bcn_pose_array, bcn_pose
...
class Test(object):
    ...
    def runner(self):
        ...
        while not rospy.is_shutdown():
            ...
            loaded = self.load_data('uwac_sim/bcn_frame_array',frame_array)
            """packet = Packet()
            packet.rosmsg_type = "uwac_sim/bcn_frame"
    ...
    def __rxPoseCallback(self, msg):
        if msg is not None:
            self.rx_pose = msg
            rospy.loginfo(f'__rxPoseCallback ({self.rx_pose.x},{self.rx_pose.y},{self.rx_pose.z})')
