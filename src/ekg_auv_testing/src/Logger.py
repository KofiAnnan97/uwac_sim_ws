...
from ekg_auv_testing.msg import VehicleLog, VehiclePose

rospack = rospkg.RosPack()
PKG_PATH = rospack.get_path("ekg_auv_testing")
LOG_PATH = os.path.join(PKG_PATH, "logs")
        ...
        try:
            if self.is_running == True and msg.vehicle_name in self.strategies:
                with open(self.file_paths[msg.vehicle_name],'a', newline='') as cw:
                    writer = csv.writer(cw)
                    writer.writerow([msg.stamp.secs, round(msg.x, 2),  round(msg.y, 2), round(msgs.z, 2)])
        except Exception as e:
            print(e)
